...
KUBESPRAY_VIRTUALENV=/local/venv/oran-kubernetes
...
PYTHONVENV=/local/venv/oran-kubernetes
...
##
/local/venv/oran-kubernetes/bin/python --version
if [ ! $? -eq 0 ]; then
    /local/venv/oran-kubernetes/bin/python3 --version
    if [ $? -eq 0 ]; then
	PYTHON=/local/venv/oran-kubernetes/bin/python3
    else
    ...
    PYTHON=/local/venv/oran-kubernetes/bin/python
fi
...
##
are_packages_installed cryptography future \
    six lxml pip
success=`expr $? = 0`
# Keep trying again with updated cache forever;
...
while [ ! $success -eq 0 ]; do
    do_apt_update
    ${PYTHONVENV}/bin/${PIP} install cryptography future six lxml pip
    success=$?
done
...
SWAPPER=adminy
NODEID=local_nodeid
PNODEID=local_pnodeid
EEID=local_eeid
EPID=local_epid
OURDOMAIN=local_domain
FULLDOMAIN=local_full_domain
NFQDN="lodal_NFQDN_`hostname`"
PFQDN="local_PFQDN_`hostname`"
MYIP=`hostname -I | cut -f2 -d' '`
EXTERNAL_NETWORK_INTERFACE=`ip -o addr show | grep "$MYIP" | awk '{print $2}'`
HOSTNAME=`hostname`
ARCH=`uname -m`
